...
class Graph(Node):
    def __init__(self):
        ...
        self.marker_node.scale.x = 0.03
        self.marker_node.scale.y = 0.03
        self.marker_node.scale.z = 0.03
        ...
        self.marker_visited.scale.x = 0.03
        self.marker_visited.scale.y = 0.03
        self.marker_visited.scale.z = 0.03
        ...
        self.marker_unvisited.scale.x = 0.03
        self.marker_unvisited.scale.y = 0.03
        self.marker_unvisited.scale.z = 0.03
    ...
    def map_callback(self, msg):
        ...
        self.inflate_obstacles(inflation_radius=0.25)
        self.create_grid()
    ...
    def create_grid(self):
        ...
        distance_threshold = self.grid_stepsize*2.0
        ...
        for nodei in self.nodes:
            for nodej in self.nodes:
                if nodei != nodej:
                    ...
                    if distance<distance_threshold and not self.is_path_obstructed(nodei, nodej):
    ...
    def inflate_obstacles(self, inflation_radius=0.2):
        ...
        for y, x in zip(*self.obstacles_):
    ...
    def is_path_obstructed(self, node1, node2, step=0.05):
        dist = node1.distance_to(node2)
        steps = int(dist / step)
        for i in range(steps + 1):
            t = i / steps
            x = (1 - t) * node1.x + t * node2.x
            y = (1 - t) * node1.y + t * node2.y
            if self.obstacle_node(x, y):
                return True
        return False
