...
from smart_factory_simulations.move_to_object import MovetoObject
...
class Robot(Node):
    ...
    def ask_for_goal(self):
        ...
        self.get_logger().info('Asking Goals..')

        # Ask for the goal method -- Once the robot starts
        start_time = time.time()
        duration = 20   
        request_goal = RequestGoal(self.robot,self.robot_name) 
        request_goal.send_request(self.robot_number)
        if request_goal.available > 0:
            status = request_goal.get_goals(self.robot_number)
        else:
            self.destroy_node()
            self.ask_for_goal()
        ...
        elif status == 'No goals at the moment':
            request_goal.destroy_node()
