...
class Graph(Node):
    def __init__(self):
        ...
        self.marker_edges_.scale.x = 0.005
        self.marker_edges_.scale.y = 0.005
        self.marker_edges_.scale.z = 0.005
        self.marker_edges_.color.a = 1.0
        self.marker_edges_.color.r = 0.0
        self.marker_edges_.color.g = 0.0
        self.marker_edges_.color.b = 0.0

        self.marker_path_ = Marker()
        self.marker_path_.header.frame_id = "map"
        self.marker_path_.ns = "path"
        self.marker_path_.type = Marker.LINE_STRIP
        self.marker_path_.action = Marker.ADD
        self.marker_path_.pose.position.x = 0.0
        self.marker_path_.pose.position.y = 0.0
        self.marker_path_.pose.position.z = 0.0
        self.marker_path_.scale.x = 0.05
        self.marker_path_.scale.y = 0.05
        self.marker_path_.scale.z = 0.05
        self.marker_path_.color.a = 1.0
        self.marker_path_.color.r = 0.0
        self.marker_path_.color.g = 1.0
        self.marker_path_.color.b = 0.0
    ...
    def create_grid(self):
        ...
        distance_threshold = self.grid_stepsize*0.1
...
class Map(Node):
    def __init__(self):
        ...
        self.map_msg = self.load_map('/home/prethivi/ros2_ws/Smart_Factory/smart_factory_ws/turtlebot3_housemap.yaml')
    ...
    def load_map(self,yaml_path):
        ...
        img_data = cv2.imread(map_yaml['image'],cv2.IMREAD_GRAYSCALE)
        ...
        for pixel in img_data.flatten():
            norm_pixel = pixel/255.0
            if norm_pixel >= free_thresh:
                occupancy_grid.append(0)
            elif norm_pixel <= occupied_thresh:
                occupancy_grid.append(100)
            else:
                occupancy_grid.append(-1)
        ...
        self.map_.info.width = width
        self.map_.info.height = height
        
        self.map_.info.origin.position.x = origin[0]
        self.map_.info.origin.position.y = origin[1]
        self.map_.info.origin.position.z = 0.0
        self.map_.info.origin.orientation.w = 1.0

        self.map_.data = occupancy_grid
...
class Astar(Node):
    ...
    def visualise_path(self,path):
        self.graph_.marker_path_.points = []
        ...
        for idx in path:
            ...
            point.y = self.graph_.nodes[idx].y
            point.z = 0.0
            self.graph_.marker_path_.points.append(point)
        
        self.graph_.marker_publish.publish(self.graph_.marker_path_)
